...
class TradingEngine:
    ...
    def _reconcile_orders_and_positions(self) -> bool:
        ...
        try:
            ...
            if tws_has_positions and not internal_has_position:
                ...
                # Reset signal state for proper re-entry flow
                if self.signal_generator:
                    if self.signal_generator.orb:
                        self.signal_generator.orb.reset_breakout_for_reentry()
                    self.signal_generator._signal_generated = False

                # Go to WAITING_FOR_BREAKOUT for proper re-entry
                if self._safe_state_transition(EngineState.WAITING_FOR_BREAKOUT, "Position state cleared - no TWS positions"):
                    # Visual separator after trade close
                    self.logger.info("")
                    self.logger.info("")
                    self.logger.info("*" * 70)
                    self.logger.info("***   TRADE CLOSED - CHECKING FOR NEXT SIGNAL   ***")
                    self.logger.info("*" * 70)
                    self.logger.info("")
                return False
    ...
    def _handle_position_monitoring(self):
        ...
        if not self.position_manager.has_open_position():
            # Position may have been closed externally - reset for proper re-entry
            self.logger.info("Position closed, resetting for new breakout signals")
            ...
            if self._safe_state_transition(EngineState.WAITING_FOR_BREAKOUT, "Position closed"):
                # Visual separator after trade close
                self.logger.info("")
                self.logger.info("")
                self.logger.info("*" * 70)
    ...
    def _handle_bracket_order_monitoring(self):
        ...
        if status['position_closed']:
            ...
            if not our_legs_on_ibkr:
                ...
                if in_breakout_window and not past_cutoff:
                    # Can take another trade - reset ORB and signal state for re-entry
                    if self.signal_generator and self.signal_generator.orb:
                        self.signal_generator.orb.reset_breakout_for_reentry()
                    # Reset signal generated flag to require fresh breakout detection
                    if self.signal_generator:
                        self.signal_generator._signal_generated = False

                    if self._safe_state_transition(EngineState.WAITING_FOR_BREAKOUT, "Bracket exit completed - Ready for new signals"):
                        # Visual separator after trade close
                        self.logger.info("")
                        self.logger.info("")
                        self.logger.info("*" * 70)
                        ...
                        self.logger.info("")
                        self.logger.info("=" * 60)
                        self.logger.info("READY FOR NEW BREAKOUT SIGNALS")
                        self.logger.info(f"Breakout window active until: {breakout_end}")
                        self.logger.info(f"Cutoff time: {cutoff_time}")
                        self.logger.info("=" * 60)
                else:
                    # Session should end
                    reason = "Past cutoff time" if past_cutoff else "Breakout window expired"
                    self._safe_state_transition(EngineState.SESSION_ENDED, f"Bracket exit completed - {reason}")
            else:
                ...
                if self._bracket_close_warning_count >= 5:
                    ...
                    if in_breakout_window and not past_cutoff:
                        ...
                        if self.signal_generator:
                            self.signal_generator._signal_generated = False
                        if self._safe_state_transition(EngineState.WAITING_FOR_BREAKOUT, "Bracket exit completed (forced recovery) - Ready for new signals"):
                            # Visual separator after trade close
                            self.logger.info("")
                            self.logger.info("")
                            self.logger.info("*" * 70)
                        ...
                        reason = "Past cutoff time" if past_cutoff else "Breakout window expired"
                        self._safe_state_transition(EngineState.SESSION_ENDED, f"Bracket exit (forced recovery) - {reason}")
    ...
    def _on_exit_triggered(self, position: Position, exit_reason: ExitReason):
        ...
        if result["success"]:
            ...
            if in_breakout_window and not past_cutoff:
                # Can take another trade - go back to waiting for breakout
                # Reset ORB breakout flag to allow new breakout detection
                if self.signal_generator and self.signal_generator.orb:
                    self.signal_generator.orb.reset_breakout_for_reentry()

                if self._safe_state_transition(EngineState.WAITING_FOR_BREAKOUT, f"Position closed: {exit_reason.value} - Ready for new signals"):
                    # Visual separator after trade close
                    self.logger.info("")
                    self.logger.info("")
                    self.logger.info("*" * 70)
                ...
                # Session should end
                reason = "Past cutoff time" if past_cutoff else "Breakout window expired"
                self._safe_state_transition(EngineState.SESSION_ENDED, f"Position closed: {exit_reason.value} - {reason}")
